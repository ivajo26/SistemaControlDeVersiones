...
SlideDeck.prototype.loadConfig_ = function(config) {
  ...
  if (this.config_.presenters) {
    ...
    if (presenters.length == 1) {
      ...
      var universidad = p.universidad ? '<span>Instituto</span>' +
          '<a href="http://universidad.com/' + p.universidad + '">' +
          p.universidad + '</a>' : '';
      ...
      var html2 = [gplus, universidad, www, github].join('<br>');
    ...
    if (dataConfigPresenter) {
      ...
      if (settings.eventInfo) {
        ...
        dataConfigPresenter.innerHTML += settings.eventInfo.title + dateInfo + ' - ' + settings.eventInfo.ko;
      }
    }
  }
